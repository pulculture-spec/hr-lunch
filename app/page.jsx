...
export default function Home() {
  ...
  const [syncStatus, setSyncStatus] = useState('loading') // loading | ok | saving | error
  const [modal, setModal] = useState(null) // { year, month, day }
  ...
  // Poll every 30s for updates from other users
  useEffect(() => {
    const interval = setInterval(fetchData, 30000)
    return () => clearInterval(interval)
  }, [fetchData])

  const saveData = useCallback(async (newSchedules) => {
  ...
  function debounceSave(newSchedules) {
    ...
    saveTimer.current = setTimeout(() => saveData(newSchedules), 600)
  }
  ...
  function setStatus(mem, st) {
    ...
    const k = key(modal.year, modal.month, modal.day)
  ...
  const syncLabel = {
    loading: '불러오는 중...',
    ok: '팀 전체 공유 중 · 30초마다 자동 새로고침',
    saving: '저장 중...',
    error: '저장 실패 — 잠시 후 재시도',
  }[syncStatus]
    ...
    <main style={{ maxWidth: 700, margin: '0 auto', padding: '1.25rem 1rem', fontFamily: 'system-ui, sans-serif' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)', gap: 4 }}>
        ...
        {Array.from({ length: daysInMonth }).map((_, i) => {
          ...
          const daySchedule = schedules[k] || {}
          const absentList = MEMBERS.filter(m => (daySchedule[m] || 'none') !== 'none')

          return (
            <div
              key={d}
              onClick={() => openModal(year, month, d)}
              style={{
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
                {!isWeekend && absentList.map(mem => (
                  <div key={mem} style={{
                    ...
                    background: (daySchedule[mem] === 'annual') ? '#FCEBEB' : '#E6F1FB',
                    color: (daySchedule[mem] === 'annual') ? '#A32D2D' : '#185FA5',
                    whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis'
                  }}>
                    {mem.slice(-2)} · {daySchedule[mem] === 'annual' ? '연차' : '약속'}
                  </div>
                ))}
                  ...
                  <div style={{ display: 'flex', gap: 6 }}>
                    {[['none','가능'], ['annual','연차'], ['other','약속']].map(([val, label]) => (
                      <button key={val} onClick={() => setStatus(mem, val)} style={{
                        ...
                        border: st === val
                          ? (val === 'annual' ? '1px solid #F09595' : val === 'other' ? '1px solid #85B7EB' : '1px solid #aaa')
                          : '0.5px solid #ddd',
                        background: st === val
                          ? (val === 'annual' ? '#FCEBEB' : val === 'other' ? '#E6F1FB' : '#f0f0ee')
                          : 'none',
                        color: st === val
                          ? (val === 'annual' ? '#A32D2D' : val === 'other' ? '#185FA5' : '#333')
                          : '#888',
                      }}>{label}</button>
                    ))}
                  </div>
...
const navBtnStyle = {
  ...
  padding: '4px 12px', fontSize: 16, cursor: 'pointer', color: '#333',
}
